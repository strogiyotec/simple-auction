...
import {airdropFn, bidSeed, pdaFn, speedCheck} from './utils'
...
describe('auction', () => {
    ...
    it('Cant place bid lower than the inital price', async () => {
        ...
        const [_pda, _] = await pda(bidSeed(state.publicKey, bidder1.publicKey))
        console.log("I am here ", _pda);
        try {
    ...
    let bidPenultimate: anchor.web3.PublicKey,
        bidderPenultimate: anchor.web3.Keypair, bidPenultimateAmount: number;
    ...
    let bidHighest: anchor.web3.PublicKey, bidderHighest: anchor.web3.Keypair,
        bidHighestAmount: number;
