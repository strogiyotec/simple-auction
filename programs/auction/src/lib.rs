...
declare_id!("5FXmbT3CoPP1LGesbtN9AC51QUx8jSjxSwMjgor6rmua");
...
pub mod auction {
    ...
    pub fn bid(ctx: Context<Bid>, amount: u64) -> Result<()> {
        ...
        msg!("Offer amount {}",offer.amount);
        let diff = amount.checked_sub(offer.amount);
        if diff == None {
            return Err(error!(Errors::InvalidOperation));
        }
        msg!("Diff is {}",diff.unwrap());
